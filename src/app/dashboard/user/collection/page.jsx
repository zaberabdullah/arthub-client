...
export default function MyCollectionPage() {
  ...
  if (loading) return (
    <div className="p-6 grid grid-cols-2 md:grid-cols-3 gap-4">
      {[...Array(6)].map((_, i) => <div key={i} className="h-52 bg-zinc-100 rounded-2xl animate-pulse" />)}
    </div>
  );
    ...
    <div className="max-w-5xl mx-auto p-6">
      ...
      {purchases.length === 0 ? (
        <Card className="p-12 text-center border border-zinc-200 dark:border-zinc-800">
          ...
          <Link href="/artworks" className="text-violet-600 text-sm hover:underline">Start collecting →</Link>
        </Card>
      ) : (
            ...
            <Link key={p._id} href={`/artworks/${p.artworkId}`}>
              <div className="group rounded-2xl border border-zinc-100 dark:border-zinc-800 overflow-hidden hover:shadow-lg transition-all bg-white dark:bg-zinc-900">
                <div className="relative h-48 overflow-hidden bg-zinc-100">
                  {p.artworkImage ? (
                    <img src={p.artworkImage} alt={p.artworkTitle} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center text-4xl">🎨</div>
                  )}
                </div>
                <div className="p-4">
                  <h3 className="font-semibold text-zinc-900 dark:text-zinc-100 text-sm truncate">{p.artworkTitle || "Artwork"}</h3>
                  <p className="text-zinc-400 text-xs mt-0.5">by {p.artistName || "—"}</p>
                  <p className="text-violet-600 font-bold text-sm mt-2">${p.amount}</p>
                </div>
              </div>
            </Link>
          ))}
