...
const TIERS = [
  ...
  { name: "Pro", price: "$9.99/mo", limit: "9 purchases", tier: "pro", color: "bg-violet-100 text-violet-700 border-violet-200" },
  { name: "Premium", price: "$19.99/mo", limit: "Unlimited", tier: "premium", color: "bg-amber-100 text-amber-700 border-amber-200" },
];
  ...
  const fetchPurchases = async () => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_SERVER_URL}/api/transactions/my-purchases`,
        { credentials: "include" }
      );
    ...
    <div className="max-w-5xl mx-auto p-6 space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        ...
        <Card className="p-5 border border-zinc-200 dark:border-zinc-800">
          ...
          <p className="text-3xl font-bold text-zinc-900 dark:text-zinc-100">
            {tier === "premium" ? "∞" : Math.max(0, (typeof currentLimit === "number" ? currentLimit : 0) - purchaseCount)}
          </p>
        </Card>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {TIERS.map((t) => (
            <Card key={t.tier} className={`p-5 border-2 ${tier === t.tier ? "border-violet-500" : "border-zinc-100 dark:border-zinc-800"} relative`}>
              {tier === t.tier && (
                <span className="absolute top-3 right-3 text-xs bg-violet-600 text-white px-2 py-0.5 rounded-full">Current</span>
              )}
              ...
              {tier !== t.tier && t.tier !== "free" && (
                <Button
                  ...
                  onPress={() => alert("Stripe coming soon!")}
                >
                  Upgrade
                </Button>
              )}
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          <Link href="/dashboard/user/purchases" className="text-sm text-violet-600 hover:underline">View all →</Link>
        </div>
        ...
        {purchases.length === 0 ? (
          <Card className="p-10 text-center border border-zinc-200 dark:border-zinc-800">
            ...
            <Link href="/artworks">
              <Button color="primary" size="sm" className="mt-4 rounded-xl">Browse Artworks</Button>
            </Link>
          </Card>
        ) : (
                ...
                <tbody>
                  {purchases.slice(0, 5).map((p) => (
                    <tr key={p._id} className="border-b border-zinc-50 dark:border-zinc-800/50 hover:bg-zinc-50 dark:hover:bg-zinc-900/50">
                      <td className="px-5 py-4 font-medium text-zinc-900 dark:text-zinc-100">{p.artworkTitle || "—"}</td>
