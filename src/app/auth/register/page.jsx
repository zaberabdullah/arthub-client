...
export default function RegisterPage() {
  ...
  const handleGoogle = async () => {
    if (!role) { setError("Please choose a role before continuing with Google."); return; }
    setError(""); setSuccess("");
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden flex-col justify-between p-12 bg-[#0D0D0D]">
        <div className="absolute inset-0" style={{ background: "radial-gradient(ellipse at 70% 80%,#1a0a2e 0%,#0D0D0D 60%)" }} />
        <div className="absolute bottom-20 right-20 w-72 h-72 rounded-full opacity-20" style={{ background: "linear-gradient(135deg,#db2777,#7c3aed)" }} />
        <div className="absolute top-24 left-8 w-40 h-40 rounded-full opacity-15" style={{ background: "linear-gradient(135deg,#f97316,#db2777)" }} />
        <div className="absolute top-1/3 right-1/4 w-56 h-56 rounded-full opacity-10" style={{ background: "linear-gradient(135deg,#7c3aed,#06b6d4)" }} />
        ...
        <div className="relative z-10 space-y-8">
          {[
            { icon: "🎨", title: "Sell your art globally",   body: "Reach collectors and buyers from every corner of the world." },
            { icon: "💳", title: "Secure Stripe payments",   body: "Every transaction is protected and processed instantly." },
            { icon: "📊", title: "Track your sales",         body: "A dashboard built to help artists grow their revenue." },
          ].map((item) => (
            <div key={item.title} className="flex items-start gap-4">
              <div className="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-lg flex-shrink-0">{item.icon}</div>
              <div>
          ...
          <Card className="w-full p-6 shadow-sm border border-zinc-200 dark:border-zinc-800">
            ...
            <div className="flex flex-col items-center justify-center gap-2 pb-6 border-b border-zinc-100 dark:border-zinc-800 mb-6 text-center">
              ...
              <div className="flex items-center gap-2 mt-1">
                {[1, 2].map((s) => (
                  <div key={s} className={`h-1.5 rounded-full transition-all ${s === step ? "w-6 bg-violet-600" : s < step ? "w-3 bg-violet-400" : "w-3 bg-zinc-200 dark:bg-zinc-700"}`} />
                ))}
              </div>
            </div>
            ...
            {step === 1 && (
              <div className="flex flex-col gap-4">
                {ROLES.map((r) => (
                  ...
                  >
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 transition-colors ${
                      role === r.value ? "bg-violet-100 dark:bg-violet-900/50 text-violet-600 dark:text-violet-400" : "bg-zinc-100 dark:bg-zinc-800 text-zinc-400"
                    }`}>
                      {r.icon}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className={`font-medium text-sm ${role === r.value ? "text-violet-700 dark:text-violet-300" : "text-zinc-800 dark:text-zinc-200"}`}>{r.label}</p>
                ...
                <div className="text-center pt-4 border-t border-zinc-100 dark:border-zinc-800 mt-1 text-sm text-zinc-600 dark:text-zinc-400">
                  ...
                  <Link href="/auth/login" className="font-medium text-violet-600 dark:text-violet-400">Sign in instead</Link>
                </div>
              </div>
            )}
            ...
            {step === 2 && (
              <form onSubmit={handleRegister} className="flex flex-col gap-5">
                ...
                <div className="flex items-center justify-between -mt-1">
                  ...
                  <button type="button" onClick={() => { setStep(1); setError(""); }} className="text-xs text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200 transition-colors underline underline-offset-2">
                    Change
                  </button>
                </div>
                ...
                <TextField isRequired name="password" className="flex flex-col gap-1.5">
                  ...
                  {strength && (
                    <div className="space-y-1 mt-0.5">
                      ...
                      <p className="text-xs text-zinc-400">Strength: <span className="text-zinc-600 dark:text-zinc-300">{strength.label}</span></p>
                    </div>
                  )}
                </TextField>
                ...
                <TextField isRequired name="confirm" className="flex flex-col gap-1.5">
                  ...
                  <InputGroup className={`flex items-center gap-2 border rounded-xl px-3 bg-zinc-50 dark:bg-zinc-900 transition-colors ${
                    ...
                    <Input
                      ...
                      className="w-full bg-transparent py-2.5 text-sm outline-none border-none text-zinc-900 dark:text-zinc-100"
                    />
                    <button type="button" onClick={() => setShowCf(v => !v)} className="text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200 transition-colors focus:outline-none">
                ...
                <p className="text-xs text-zinc-400 dark:text-zinc-500 leading-relaxed -mt-1">
                  ...
                  <Link href="/terms" className="text-violet-600 dark:text-violet-400 underline underline-offset-2">Terms of Service</Link>{" "}and{" "}
                  <Link href="/privacy" className="text-violet-600 dark:text-violet-400 underline underline-offset-2">Privacy Policy</Link>.
                </p>
...
function UserIcon() {
  return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="text-zinc-400 pointer-events-none flex-shrink-0"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>;
}
function MailIcon() {
